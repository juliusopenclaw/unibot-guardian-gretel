...
class BrowserExtensionV2Tests(unittest.TestCase):
    ...
    def test_v2_sidepanel_has_three_workflows_native_transport_and_a0_a4(self) -> None:
        ...
        self.assertIn('nativeRequest("session.stop"', script)
        self.assertIn('nativeRequest("session.transfer.prompt"', script)
        self.assertIn('nativeRequest("session.transfer.record"', script)
        ...
        self.assertIn('id="stopSession"', html)
        ...
        self.assertIn('id="showTransferTask"', html)
        self.assertIn('id="transferAnswer"', html)
        self.assertIn("nicht bewertet", html)
    ...
    def test_content_script_extracts_active_notebook_cell_not_output(self) -> None:
        ...
        self.assertIn(".cell.notebook-cell.focused", content)
        self.assertIn("pre.monaco-colorized", content)
        self.assertIn(".cell-contents", content)
