...
- `POST /api/unibot/pilot/clearance-receipt-template`
- `POST /api/unibot/pilot/launch-gate`

## Purpose

The protocol prepares a voluntary formative pilot with synthetic Python-neuro
practice tasks. It exists so the bachelor-thesis-level research artifact can be
reviewed before any participant study starts. It is not a university
submission, ethics approval, or participant recruitment authorization.
...
The route `/api/unibot/pilot/clearance-receipt-template` returns a public-safe
empty checklist with every field set to `False`. The route
`/api/unibot/pilot/launch-gate` accepts the filled field object under
`clearance_receipt` and returns only safe status, hash, contract, and missing
field information. It rejects non-object receipts and never returns raw approval
text.
